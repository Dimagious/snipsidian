...
- **`UIStateManager`'s debounced group-open save could produce an unhandled promise rejection.** `setGroupOpen`'s 250ms-coalesced save (`ui-state.ts`) called `void this.persist()` with no rejection handling, unlike the direct toggle path in `SnippetsTab.setGroupEnabled`, which already wraps its save in try/catch. A failed `saveSettings()` on that timer fired detached from any caller, so in a real vault a save failure here was a silent unhandled rejection, not just a Notice miss. Caught and logged (`console.error("[snipsy] failed to save group state", err)`) instead of left to escape. Found via a CI-only flake (2 unhandled rejections from a test's rejecting `saveSettings` mock still wired when the debounce timer later fired) — fixed at both the production call site and the two tests that left the timer pending past their body.

### DX

- **`getSettingDefinitions()` on the settings tab** (0.4.1's `obsidianmd/settings-tab/prefer-setting-definitions`, which flags `PluginSettingTab` subclasses missing the method — new in Obsidian 1.13). Implemented but deliberately returns `[]`: a non-empty return makes Obsidian 1.13+ stop calling `display()` entirely and render the tab declaratively instead, and this plugin's whole tab strip (Snippets / Packages / General / About) lives inside `display()` — only General's scalar fields would map to real `SettingDefinition`s, so shipping those alone would silently delete the other three tabs for anyone on Obsidian 1.13+. `minAppVersion` stays at 1.5.0; a full `type: 'page'`-based migration covering the whole tab strip is tracked as a follow-up, not attempted here.
- Bumped `eslint-plugin-obsidianmd` to `^0.4.1` and the `obsidian` typings package to `^1.13.1` (dev-only; `main.js` still externalizes `obsidian` at build time, no runtime effect). `obsidian@1.13.1` exact-pins peer versions of `@codemirror/state`/`@codemirror/view`, so those two devDeps are now pinned to the exact `6.5.0`/`6.38.6` obsidian requires (down from the caret ranges `^6.5.2`/`^6.38.2`) — this keeps `npm ci` resolvable without `--legacy-peer-deps`; `@codemirror/*` is still externalized at build time, so the pin has no bundle-size or runtime effect. `eslint.config.js` adds the new rules the scanner now runs (`prefer-window-timers`, `settings-tab/prefer-setting-definitions`, `@typescript-eslint/no-misused-promises`) and ignores dirs outside `tsconfig`'s `include` (`src/**/*.ts` only) that the type-aware rules can't parse (`.obsidian-unpacked/**` — local/untracked vault artifacts — plus the git-tracked `e2e-vault.pristine/**`, `e2e/**`, `playwright.config.ts`, none of which ship in `main.js`) so `npm run lint` is green again instead of drowning in ~40k errors from unpacked vault/build artifacts. `scripts/scorecard-check.cjs`'s local mirror of the scanner flips its timer-pattern direction to match and adds a presence check for `getSettingDefinitions(` on the settings tab.
...
- **"Export snippets" is no longer a silent no-op on mobile** (B-144). The blob-anchor `<a download>` trick `BasicTab.exportJson` used doesn't work in iOS WKWebView (and other mobile WebViews) — nothing happened, no error, no Notice. On `!Platform.isDesktop` the export now writes into the vault root instead (`snipsidian-snippets.json`, retrying once with a timestamped name on a collision), with a clean Notice either way; if the vault write fails twice, it falls back to the system clipboard. Desktop's download path is unchanged. 5 new mount tests (desktop unaffected, vault write + Notice, collision retry, clipboard fallback, both-fail honest failure Notice), all failing red against the pre-fix code.
- **Install/reinstall/import Notices no longer overclaim the count when the user kept some or all conflicts as-is.** `PackageBrowser.applyResolvedInstallation` and `EspansoSection`'s conflict path both reported the full pack size regardless of what the user actually chose in the conflict-preview modal — a "Reinstall, keep everything" run said "Installed N snippets" when zero snippets changed. Both now derive the count from the diff plus the resolved choices (`core/install-plan.countAppliedChanges`: every added entry, plus every conflict whose resolved value differs from what was already stored); an all-kept reinstall/re-import now shows an honest `No changes — "<name>" already matches your library` Notice instead. New tests on the pure counter plus mount-level assertions on both install paths (all-kept, all-overwritten, and mixed added+kept scenarios).
- `ConfirmModal`'s message now respects the `\n\n` paragraph breaks callers already write into it (e.g. the "…this cannot be undone." line in the Snippets-tab and Packages-tab delete/uninstall confirmations) — added `white-space: pre-wrap` on `.confirm-message`, which had no rule at all and was collapsing every deliberate line break into one run-on sentence.
- README's "How expansion works" section no longer overclaims `$1`/`$2` as Tab-navigable tabstops — they're picker-preview highlighting, and `$1` receives the selected text when wrapping a selection with a snippet; there is no Tab-navigation anywhere in the engine.
- "Set hotkey" now prefills the Hotkeys tab's search box with the command's display name (the established community pattern) instead of only scroll-hunting for an undocumented `data-id` attribute — on a miss, the user used to land on the full, unfiltered command list with no clue which row was meant. The scroll-into-view stays as a best-effort addition on top.

### Changed

- **Unified the grouped-install pipeline** (B-140). `EspansoSection` used to hand-copy `PackageBrowser.installPackage`'s validate → collision-filter → diff → preview → write sequence, and the two copies had already diverged — this is exactly the duplication shape that produced S-009 (a security gate added on one path only). Both now call one shared, pure planner (`core/install-plan.planGroupedInstall`). **User-visible semantics change:** Espanso's stricter same-key collision check is gone — re-importing a snippet you'd previously edited at the same grouped key now goes through the same conflict preview (keep current / overwrite) that community packages already use, instead of a hard "trigger name collision" refusal. The cross-group hard-refusal (the same bare trigger living in a *different* group with a *different* replacement) is unchanged on both paths. 6 new boundary tests on the planner pin the reconciled semantics (same-key-any-value never collides, cross-group-different-value always does, cross-group-same-value never does), plus a dedicated red→green regression test on the Espanso side proving the old stricter behavior is gone.

### DX

- **Wiring tests for the two most destructive, coverage-excluded UI surfaces** (B-142). `SnippetsTab.ts` bulk delete/move and `PackageBrowser.ts` install/uninstall/reinstall had well-tested pure logic underneath but zero net on the click→plan→write wiring itself — precedent for exactly this class of bug: B-118 and B-017 both shipped through gaps like this. New mount tests drive the real selection-mode UI (bulk delete/move, including a cancel-writes-nothing case) and the real package-row click flow (install/uninstall/reinstall, including the B-135 keydown-hijack guard). Both files stay coverage-excluded — they're large render trees with plenty of untested surface left — but the vitest.config.ts comments now point at this landing instead of a stale "follow-up" claim that didn't exist anywhere in the backlog.
- **Dead-code sweep** (B-143). Removed `loadDynamicCommunityPackages` (`services/community-packages.ts`) — a vault-backed loader that was unreachable in production (its only caller always passed a `plugin`, so the router's fallback branch never ran outside tests) and carried the repo's only `process.env.NODE_ENV === 'test'` sniff. `loadAllCommunityPackages`'s `plugin` parameter is now required and the dead branch is gone; `community-cache.ts` and `community-packages.ts` no longer need a coverage exclusion (both are 100% covered by their existing suites now that the untestable dead path is gone). Also deleted: `src/catalog/builtin-markdown.yaml` (zero references anywhere in the repo), `src/app/settings-tab.ts` (a one-line re-export with zero importers), and `src/importers/espanso.ts` (a duplicate one-line re-export of `src/packages/espanso.ts`, also with zero production importers) — each removed alongside its own now-pointless test file.
- `src/test/factories/editor.ts`'s `MockEditor.replaceSelection` now delegates to `replaceRange` instead of splicing multi-line replacement text into a single line verbatim — the old version could produce a "line" that itself contained `\n` characters, an editor state no real Obsidian document can be in. One unit test in `obsidian-editor.test.ts` that had been asserting that impossible geometry now pins the real, line-split result instead.
...
- **Opt-in trigger-prefix mode** (B-137). A new "Expansion" section on the General tab: "Require a prefix before triggers" toggle (off by default) + a `:`/`;` dropdown. With it on, `todo` stays plain prose and only `:todo` (or `;todo`) expands — the prefix is consumed by the replacement, stored trigger keys stay bare, and packs/imports/picker are untouched (no migration). One global mode, deliberately no per-snippet opt-out. The Add-snippet modal's Trigger field no longer teaches the misleading `:hello` convention while silently storing a bare word; it now shows a live "Will expand when you type: …" hint (prefix-aware) whenever normalization changes what was typed. 12 new engine boundary tests in `match.test.ts` (prefix consumption geometry, double-prefix, wrong prefix char, the 64-char lookback edge, the Enter-separator path, the full mode-off regression suite unaffected) + 23 mount tests across `BasicTab.test.ts` (toggle/dropdown wiring, 7) and `Modals.extra.test.ts` (hint computation + rendering, 16).
- **Per-group enable/disable** (B-138). A toggle on each group's header in the Snippets tab mutes a group without the destructive uninstall/reinstall round-trip — disabled means the group's snippets neither expand nor appear in the snippet picker, though they stay visible (dimmed) and editable in the Snippets tab itself. Additive `disabledGroups: string[]` on settings; `getDict`/`getAllSnippetsFlat` skip disabled groups, with the trigger-collision winner correctly falling through to the next candidate group rather than just vanishing. Deleting a group cleans up its `disabledGroups` entry too. 18 new tests: 9 store-boundary (`snippets.test.ts`) + 9 mount (`SnippetsTab.test.ts`, including the toggle round-trip and the accordion-doesn't-also-collapse guard).
- **Espanso import now reports what it can't map, instead of silently corrupting it** (B-139). `espansoYamlToSnippets` returns `{ snippets, skipped }` — matches using forms, shell/script/choice variables, regex triggers, case propagation, image matches, or markdown/html-only content used to import as literal `{{broken}}` text with zero indication anything was wrong; they're now skipped with a specific reason, and the Espanso import section surfaces a summary ("N imported, M skipped: trigger1, trigger2, trigger3, … — use forms/scripts Snipsy doesn't support") both as a status line and, for conflicting imports, inside the confirm modal. Three free-win mappings apply before anything is judged unsupported: Espanso's `$|$` cursor placeholder → `$|`, `{{clipboard}}` → `$clipboard`, and a single plain `date`-type variable → `$date`/`$time`. A pack that resolves to nothing importable shows a clear "Nothing to import" message and writes nothing. 24 new tests in `espanso.test.ts` (every skip reason, both free-win mappings, mixed/all-skipped/zero-skip partitioning) + 5 new mount tests in `EspansoSection.test.ts`.

### Changed

- **The expansion hot path now gates on genuine typing, not "any document change"** (B-149, folds in B-141). `cm6-bridge.ts` used to react to every `workspace.on("editor-change")` event with no check of where the change came from — undo, redo, paste, drag-drop, and another plugin's programmatic edit all looked identical to "the user just typed a separator." It's now a registered CM6 `EditorView.updateListener` extension that only attempts expansion when a transaction's `userEvent` annotation indicates real typing (character input or Enter/newline insertion) and the view isn't mid-IME-composition. **Behavior changes as a result, by design:** pasting text that ends in `trigger ` no longer expands it; undo/redo never re-triggers an expansion; typing while composing IME input (CJK languages) never dispatches an edit mid-composition. The gate predicate is a pure, independently-tested function (`change-source-gate.ts`, 23 tests) extracted specifically so "fire or not" is verifiable without a real CM6 view. New e2e coverage in `expansion.spec.ts`: undo-after-an-Enter-triggered-expansion (mirrors the existing space-separator case), paste-ending-in-a-trigger does not expand, and a backspace that merges lines does not phantom-expand a trigger sitting at the end of the previous line.
...
- **Snippet picker no longer overflows phone-width screens; removed CSS for a class nothing ever applied** (B-134). `.snippet-picker-modal { min-width: 520px }` forced the picker wider than an iPhone viewport, breaking the plugin's headline mobile flow (`isDesktopOnly: false`). A separate viewport cap had been designed on a `.snipsy-picker` class, but no TypeScript ever added that class — dead CSS since it shipped. Folded the intent into the live rule instead (`min-width: min(520px, 100%)`, viewport-bounded `max-width`) and deleted the dead `.modal.snipsy-picker` block. The keyboard-navigation hints row ("↑/↓ to navigate…") is now hidden under `@media (pointer: coarse)` — meaningless on touch. Mount test pins that the modal element still carries the class the CSS targets.
- **Keyboard users can install/uninstall community packages again** (B-135). A package row's `keydown` handler called `preventDefault()` + opened the details modal on Enter/Space with no check of the event target. Keydown bubbles up from the row's inner Install/Uninstall buttons regardless of their click handlers' `stopPropagation()` (that only stops `click`), so tabbing onto Install and pressing Enter had the row cancel the browser's native button-activation default action before the button's own handler ever ran — install silently never fired and details opened instead. The keydown decision is now a small pure, exported `shouldOpenPackageDetailsOnKeydown` (guards on `event.target === row`) so the hijack case is unit-tested without mounting the network-backed component; `PackageBrowser.ts` has no other tests and stays coverage-excluded. 4 new tests in `PackageBrowser.keydown.test.ts`, failing red pre-fix (verified via `git stash`).
- **`$clipboard` expansion can no longer land on the wrong line** (B-136). `tryExpandAtSeparator` snapshots the trigger's line, then `await`s `expand(...)` — a real await when the replacement contains `$clipboard` (`readClipboard` hits `navigator.clipboard`). The old `applyEditPlan` re-read `editor.getCursor()` *after* that await and applied the edit to whatever line the cursor was on by then, so pressing Enter or clicking elsewhere during a slow clipboard read corrupted unrelated text. The target line is now captured before the await and threaded through explicitly; a cheap staleness guard also skips the edit if the trigger text at the captured range no longer matches what the plan expected (e.g. a fast backspace-and-retype during the same window). 2 new tests in `obsidian-editor.test.ts` with a deferred `readClipboard` promise, both failing red pre-fix (verified via `git stash`).
- **Expansion no longer silently no-ops when Enter is the separator** (B-148, regression exposed by B-146's e2e repair). On current Obsidian/CM6, pressing Enter right after a trigger reports the cursor at column 0 of the brand-new line, not at the end of the line the trigger lived on — `tryExpandAtSeparator`'s `if (cursor.ch === 0) return;` guard silently swallowed every Enter-terminated trigger (`e2e/expansion.spec.ts`'s pre-existing B-109 spec caught this the first time it actually ran). The function now recognizes cursor-at-column-0-of-a-later-line as "the line break is the separator" and checks the end of the *previous* line for the trigger. `makeExpandInput`/`makeContext` gained optional `line`/`ch` overrides (default to the live cursor, so every other call site is unaffected) so the fenced-code/frontmatter/inline-code context checks evaluate against the line the trigger actually lived on. 3 new unit tests in `obsidian-editor.test.ts` (expands on Enter, no-ops on a non-trigger previous line, still no-ops at column 0 of the first line), failing red pre-fix (verified via `git stash`); `e2e/expansion.spec.ts`'s B-109 spec now passes for the first time this can be confirmed.
- **Async `$clipboard` expansion no longer teleports the cursor away from wherever the user moved it** (B-147). `applyEditPlan`'s `setCursor` repositioned unconditionally after the text edit — including when the user had navigated to a different line during the clipboard `await` (B-136 already stopped the *text* from landing on the wrong line, but the cursor jump remained). It now reads the live cursor before the edit and skips the reposition — never the text edit itself — when the user is no longer on the captured line. This is also what makes the B-148 Enter-as-separator fix leave the cursor where Enter put it (a new, later line) instead of yanking it back onto the line above. Extended the B-136 regression test in `obsidian-editor.test.ts` with a cursor-position assertion, failing red pre-fix (verified via `git stash`).

### Tests

- **Undo-after-expansion verified NOT to be a trap — pinned as a green e2e contract** (B-132, unblocked by B-146's e2e repair). The theoretical risk: the expansion hot path decides purely from document state (cursor position + last-typed character) with no check of whether an `editor-change` came from real typing vs. undo/redo/paste, so undoing an expansion could look identical to a fresh trigger+separator keystroke and re-expand instantly. Verified empirically once e2e could actually run: CM6 groups the plugin's programmatic `replaceRange` into the *same* undo transaction as the triggering keystrokes (when they land within its history-grouping window), so a single Cmd+Z undoes the typed trigger and its expansion together, atomically — the "trigger present, not yet expanded" intermediate state the trap would need is never actually visited. `e2e/expansion.spec.ts`'s B-132 spec (rewritten around this finding — an anchor line + a wait past CM6's grouping window give undo a deterministic single step to land on) is green on repeated runs. No source change was needed; the change-source guard considered in the original plan is not required.

### DX

- **`electron` devDependency bumped `31.7.7` → `39.8.3` to match Obsidian's bundled runtime** (B-146). Local e2e was dead: the installed Electron binary had a stale/invalid code signature (SIGKILLed on launch — `com.apple.provenance` only in its `xattr`, most likely from a macOS update) and, independently, was three majors behind Obsidian 1.12.7's bundled Electron 39.8.3. A fresh `npm i -D electron@39.8.3` replaces the binary and closes the version gap. Dev-only dependency — no runtime/bundle impact. Restores the entire local e2e suite (previously 100% blocked); see the B-132/B-148 entries above, both only findable once this was fixed.
...
- **Engine hot-path no longer phantom-matches inherited prototype keys** (S-008). `findTrigger` decided "is this a known trigger?" with `dict[trigger] === undefined`, which resolves inherited `Object.prototype` members — so typing an everyday word like `constructor`, `toString`, `valueOf`, `hasOwnProperty`, `isPrototypeOf` or `__proto__` before a separator reported a phantom match. `expand` then read `dict[trigger]` (a function / the prototype object), called `.replace` on a non-string, and threw a `TypeError` on every such keystroke — caught and logged by the `cm6-bridge` boundary, so the visible symptom was console-error spam and a dead keystroke rather than a crash. No malicious input required; a Snipsy user (often a developer taking code notes) hits it by typing "constructor ". Fixed with `Object.prototype.hasOwnProperty.call(dict, trigger)` — the read-side counterpart to the write-side defence already in `ui/utils/group-utils.ts` (S-004). Regression tests in `engine/match.test.ts` (phantom-match rejected for six prototype names; a real own `toString` snippet still matches) and `engine/expand.test.ts` (resolves to null, no throw).
- **Espanso YAML import now enforces the install-time limits** (S-009). The community-package install path gates on `validatePackageForInstall` (count ≤ 500, replacement ≤ 10000 chars, total ≤ 2 MiB, trigger charset), but `EspansoSection` — which imports YAML *pasted from an untrusted source* — wrote straight to `settings.snippets` with no limit at all, nullifying the entire limit system on this write path (the most likely real bypass per the audit brief). Now the parsed bare-trigger map is run through `validatePackageForInstall` before the diff/write, rejecting oversized replacements, count-cap overruns, and `/`-bearing or otherwise malformed triggers with a Notice and no mutation. Regression tests in `EspansoSection.test.ts`.
- **GitHub Contents-API listing entries are shape-checked before use** (S-011). The directory-listing JSON was parsed and cast to `GitHubContentEntry[]` on trust; a malformed or spoofed `api.github.com` response (entry missing `name`, or a non-string `name`) made `f.name.endsWith(...)` throw a `TypeError` *outside* the surrounding `JSON.parse` try/catch, so one bad entry rejected the whole community-pack load instead of failing gracefully. The filter now validates `name`/`download_url` are strings before touching them (download URLs are still allowlist-checked in `fetchAndParsePack`, S-005). Regression test in `community-packages.test.ts`.

Audit also reviewed, no code change: JSON-backup import limits (S-010 — deliberately *not* count-capped, since a user's own backup can legitimately exceed 500 snippets; the S-008 read-guard neutralises the dangerous-key runtime risk and `isRecordOfString` already rejects non-string values); `$clipboard` auto-read on expansion (S-012 — privacy note: an imported snippet containing `$clipboard` silently pulls the clipboard into the note when expanded; the literal `$clipboard` text is visible in the install diff preview, so flagged for documentation rather than a behaviour-changing gate); ReDoS on the per-keystroke path (clean — `findTrigger` is a bounded character walk, placeholder substitutions are linear); `github-issue-url.ts` (clean — `URLSearchParams` encodes user content, host is a constant).
...
- **New `npm run scorecard:check` gate** wired into `release:check` and the CI workflow. Locally-runnable mirror of the scorecard scanner — fails the build on the same patterns the public scanner flags: domain-assembly via `.join(".")`/`.join("/")` on inline string arrays, bare `document.*`/`window.*`/`setTimeout(`/`setInterval(` (excluding the legitimate `window.open` browser-tab pattern), `el.createEl("div"|"span", {...})` instead of `createDiv`/`createSpan`, `@ts-expect-error`/`@ts-ignore`, `!important` in source CSS, manifest description starting with the plugin name. Test files are skipped (they don't ship in the bundle). Future scorecard regressions of these shapes fail the release commit *before* the public score drops. The mismatch this PR found between local-lint config and what scorecard scanner actually checks was the root cause — closing that gap.

- **Community-packages cache no longer pins blank state for 24h after a transient failure** (B-118 — surfaced during 1.1.7 mobile testing). Pre-fix: a single failed GitHub fetch — 403 rate-limit, network error on iOS, any non-2xx — caused `loadCommunityPackagesWithCache` to persist `{ packages: [], lastUpdated: now }`, overwriting whatever valid cache had been there. The user got a blank Packages tab and the 24-hour TTL guaranteed they stayed blank until the next day. The Refresh button "worked" (state→ready) and fired a misleading "Community packages refreshed" Notice on an empty result. Fix is two-layered. `loadCommunityPackagesFromGitHub` now returns a discriminated `{ ok: true, packages } | { ok: false, reason }` so the cache layer can tell "live load really succeeded with empty catalog" from "live load failed and we have no data" — the former still persists (404 / empty directory is a real catalog state), the latter is now strictly read-only against the existing cache. On failure, callers see the previously-cached packages with `source: "fallback"` and an `error` reason. The Packages-tab Refresh Notice keys off `source`: "Refreshed N packages" / "Refreshed — no community packages available" on real live success, "GitHub rate-limited; showing N cached packages" / "Network unavailable; no cached packages to show" on fallback. 9 new boundary tests cover all five fail modes (`rate_limited` / `not_found` / `http` / `network` / `parse`) + the cache-preservation invariant.
...
- **E2E coverage filled in for four boundary cases** (B-108 / B-109 / B-110 / B-111). `expansion.spec.ts` gets three new tests pinning the keystroke → engine wiring at corners that have historically slipped through: trigger at column 0 of a non-first line (line>0/ch=0 corner of `shouldExpandHere`), newline-as-separator (Enter rather than space; different `lastTyped` value), and backspace-inside-a-partial-trigger (cursor-shift before separator). `persistence.spec.ts` gets two new tests pinning the edit + delete round-trips through `saveSettings` — the existing reload test only covered add. A regression in either path is silent data loss for users; worth the ~5s per test cost.
- **`src/ui/**` is in the coverage gate now** (B-114 — per [ADR-0005](.claude/brain/decisions/0005-test-philosophy.md), opening the gate makes uncovered surface visible as a gate failure). The Obsidian test stub gains a proper fluent `Setting` builder (`setName` / `setDesc` / `setHeading` / `addText` / `addTextArea` / `addToggle`) plus `TextComponent`, `TextAreaComponent`, and `ButtonComponent` wrappers around real `<input>` / `<textarea>` / `<button>` nodes, and the DOM polyfill picks up `HTMLElement.hide()` / `show()` — enough scaffolding to mount modals in jsdom without rewriting production code. On that scaffold: 24 new mount tests across `JSONModal` / `ConfirmModal` / `TextPromptModal` / `AddSnippetModal` / `GroupPickerModal` (43% → 93.71% on `Modals.ts`); 11 new tests for the B-045 group-name flow in `EspansoSection` (0% → 89.84%) covering `nextDefaultGroupLabel` auto-increment, slug-collision skip, and the cross-group collision Notice; smoke tests for `CommunityTab.ts` and `settings.ts` re-export (both 0% → 100%); two boundary tests on `group-utils` for the `bulkMoveKeys` skip path + class delegate (88% → 100%). Final aggregate: lines **95.15%**, branches **90.13%**, functions **91.09%**, statements **95.15%** (was 88.27% lines under the old wildcard). Nine files still carry file-level excludes — each one is now an explicit, named follow-up in the brain backlog instead of being hidden inside `src/ui/**`.

### DX

- **Expansion errors are now logged instead of silently swallowed** (B-020). A broken snippet — invalid `$date` format, future regex-trigger compile failure — used to disappear on the `editor-change` handler: no expansion, no log, nothing to debug from. Now wrapped in try/catch with `console.error("[snipsy] expansion error", { filename, error }, err)`. Catch does NOT re-throw — propagating into Obsidian's event chain risks killing other plugins' handlers, so we fail-open at the boundary.
- **`getDict` first-wins collision contract documented** (B-023). The "first key by alphabetical full-key sort wins" behaviour now has a proper JSDoc block on the function explaining the resolution order, the install-time gates that prevent users from unintentionally landing in this resolution, and the tests that pin the tiebreaker.
- **`e2e/record.spec.ts` guarded against unattended runs** (B-113). The recording spec calls `win.pause()` which hangs forever waiting for a human; now skipped unless `E2E_RECORD=1` is set (which `npm run e2e:record` wires). `--grep-invert` in the `e2e` script stays as belt-and-braces for direct `npx playwright test` invocations.
