...
- **Undo-after-expansion is now a pinned e2e contract** (B-132). The expansion hot path decides purely from document state (cursor position + last-typed character), with no check of whether an `editor-change` came from real typing, undo, redo, or paste — the theoretical risk was that undoing an expansion (e.g. `brb ` → `be right back `) would look identical to a fresh trigger+separator keystroke and re-expand instantly, turning undo into a trap. Added `e2e/expansion.spec.ts` — types `brb `, undoes with Cmd+Z, asserts the document returns to `brb ` and stays that way. Could not be executed in this environment (Electron fails to attach a GUI session — even the pre-existing `canary.spec.ts` smoke test hangs identically here), so the result is unknown; the test ships as a pinned contract for the next run where e2e can execute, with no speculative code change made against an unconfirmed bug.
